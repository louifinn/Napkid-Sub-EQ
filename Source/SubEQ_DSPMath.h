...
// Conservative maximum group delay of a causal FIR, in samples.
// Frequency-domain phase method (N log N via the shared FFT, so it stays fast
// for long FIRs): unwrapped phase -> numerical differentiation. Frequency bins
// whose unwrap is ambiguous (true group delay > N/2 there) are excluded; if
// every meaningful bin is ambiguous the full length is returned (conservative,
// so PDC never under-reports).
inline int computeMaxGroupDelay(const std::vector<float>& coeffs)
{
    const int n = static_cast<int>(coeffs.size());
    if (n < 4)
        return 0;

    // Frequency response via N-point FFT
    std::vector<std::complex<double>> spec(n);
    for (int i = 0; i < n; ++i)
        spec[i] = { static_cast<double> (coeffs[i]), 0.0 };
    fftInPlace(spec.data(), n);

    const int numPoints = n / 2;
    std::vector<double> phase(numPoints + 1);
    for (int k = 0; k <= numPoints; ++k)
        phase[k] = std::atan2(spec[k].imag(), spec[k].real());

    // Unwrap; flag bins where the step exceeds pi (ambiguous for tau > n/2)
    std::vector<bool> ambiguous(numPoints + 1, false);
    for (int i = 1; i <= numPoints; ++i)
    {
        double d = phase[i] - phase[i - 1];
        if (d > 3.14159265358979323846)
        {
            phase[i] -= 2.0 * 3.14159265358979323846;
            ambiguous[i] = true;
        }
        else if (d < -3.14159265358979323846)
        {
            phase[i] += 2.0 * 3.14159265358979323846;
            ambiguous[i] = true;
        }
    ...
    // Group delay by central differences: tau(w) = -d phi / d w, dw = 2 pi / n
    const double dw = 2.0 * 3.14159265358979323846 / n;
    double maxDelay = 0.0;
    int reliable = 0;

    for (int i = 1; i < numPoints; ++i)
    {
        if (ambiguous[i] || ambiguous[i - 1] || ambiguous[i + 1])
            continue;

        const double delay = -(phase[i + 1] - phase[i - 1]) / (2.0 * dw);
        if (delay > 0.0)
        {
    ...
    // Endpoints (forward differences)
    if (!ambiguous[0] && !ambiguous[1])
    {
        const double d0 = -(phase[1] - phase[0]) / dw;
        if (d0 > 0.0) { maxDelay = std::max(maxDelay, d0); ++reliable; }
    }
    if (!ambiguous[numPoints - 1] && !ambiguous[numPoints])
    {
        const double dEnd = -(phase[numPoints] - phase[numPoints - 1]) / dw;
        if (dEnd > 0.0) { maxDelay = std::max(maxDelay, dEnd); ++reliable; }
    }

    if (reliable == 0)
        return n - 1;   // everything ambiguous: conservative full length

    if (std::isnan(maxDelay) || std::isinf(maxDelay) || maxDelay < 0.0)
        return 0;
    if (maxDelay > static_cast<double>(n - 1))
        return n - 1;
