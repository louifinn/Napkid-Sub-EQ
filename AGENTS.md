...
面向超低频（0.5 Hz ~ 500 Hz）的 8 节点参数均衡器 VST3 音频插件（C++ / JUCE 7.x / VS2022，仅 Windows）。GPL v3。当前版本 0.3.2。
...
无自动化测试框架；回归测试为独立可执行 `Tests/subeq_fft_test.cpp`（无 JUCE 依赖，88 项：FFT/twiddle 多槽缓存/biquad 系数·状态·响应·设计/overlap-add 卷积原语/FIR 设计/FilterType 映射/频谱数学/频谱 choice 映射/坐标映射/节点交互规则/弹簧积分器）：
`cl /EHsc /std:c++17 /O2 Tests/subeq_fft_test.cpp /Fo:Tests\subeq_fft_test.obj /Fe:Tests\subeq_fft_test.exe && Tests\subeq_fft_test.exe`（需 VS 开发环境）。变更验证 = 编译 + 测试 + 人工。
